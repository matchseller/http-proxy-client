...
//读取http响应
func (r *Response) Read() (PData, error) {
	//先读取响应行和响应头，分析连接类型
	r.end = 1024*200	//最大200KB的请求行和请求头
	rLen, err := (*r.conn).Read(r.buff[:r.end])
	if err != nil {
		return PData{}, fmt.Errorf("connection closed unexpectedly")
	}
	r.end = rLen
	...
	if err != nil {
		return PData{}, fmt.Errorf("parse response line error:%v", err)
	}
	if !isOk {
		return PData{}, fmt.Errorf("response line is too large")
	}

	//解析响应头
	isOk = r.parseHeader()
	if !isOk {
		return PData{}, fmt.Errorf("response header is too large")
	}

	if val, isOk := r.Header["Connection"]; (isOk && val == "close") || strings.ToUpper(r.Line["version"]) == "HTTP/1.0" {
		for {
			rLen, err := (*r.conn).Read(r.buff[r.end:])
			r.end += rLen
			if r.end == r.buffLen {
				//缓冲区的容量存不了一条请求的数据
				return PData{}, fmt.Errorf("response is too large")
			}
			if err != nil {
				r.Body = string(r.buff[r.start:r.end])

				if _, isOk := r.Header["Content-Length"]; isOk {
					r.BinaryData = string(r.buff[:r.end])
				}else{
					//加上 Content-Length 头部
					httpStatus, _ := strconv.ParseInt(r.Line["status"], 10, 64)
					tmpBuff := response.NewResponse(int(httpStatus), r.conn).SetVersion(r.Line["version"]).SetHeader(r.Header).SetBody(r.Body).GetBuff()
					r.BinaryData = string(tmpBuff)
				}
				return r.PData, nil
			}
		}
		...
		for {
			...
			if _, isOk := r.Header["Content-Length"]; isOk {
				...
				if err != nil {
					return PData{}, fmt.Errorf("parse response body error:%v", err)
				}
				//读取http请求体未成功
				if !isOk {
					r.reset()
			...
			if r.end == r.buffLen {
				//缓冲区的容量存不了一条请求的数据
				return PData{}, fmt.Errorf("response is too large:%v", r)
			}
			rLen, err := (*r.conn).Read(r.buff[r.end:])
			if err != nil {
				//连接关闭了
